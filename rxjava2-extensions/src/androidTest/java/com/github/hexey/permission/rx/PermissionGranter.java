...
public class PermissionGranter {
    ...
    public static void allowPermissionsIfNeeded(String permissionNeeded) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
                !hasNeededPermission(InstrumentationRegistry.getTargetContext(),
                    permissionNeeded)) {
    ...
    public static void deniedPermissions(String permission) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
                !hasNeededPermission(InstrumentationRegistry.getTargetContext(), permission)) {
                sleepThread(PERMISSIONS_DIALOG_DELAY);
                UiDevice device = UiDevice.getInstance(getInstrumentation());
                UiObject allowPermissions = device.findObject(new UiSelector()
                    .clickable(true)
